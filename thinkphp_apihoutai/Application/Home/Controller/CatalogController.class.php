...
class CatalogController extends BaseController {
	...
	public function edit() {
		...
		$Catalog = D("Catalog")->where(" cat_id = '$cat_id' and status = 1 ")->find();
		if ($Catalog) {
	...
	public function save() {
		...
		$return = D("Catalog")->where(" cat_id = '$cat_id' and status = 1 ")->find();
		if (!$return) {
	...
	public function catList() {
		$item_id = I("item_id");
		if ($item_id > 0) {
			$ret = D("Catalog")->where(" item_id = '$item_id' and status = 1 ")->order(" 'order', addtime asc  ")->select();
		}
		if ($ret) {
			$this->sendResult($ret);
		} else {
	...
	public function delete() {
		$cat_id = I("cat_id") ? I("cat_id") : 0;
		$cat = D("Catalog")->where(" cat_id = '$cat_id' and status = 1 ")->find();
		$item_id = $cat['item_id'];
